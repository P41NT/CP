...
void solve() {
    ...
    bool flag_moved_last = false;

    if (s[s.size() - 1] == '1') flag_moved_last = true;

    if (s.size() - 1 - last <= m && s[s.size() - 1] != '1') {
        ans -= 10;
        if (last == 0) ans += 1;
        m -= s.size() - 1 - last;
        flag_moved_last = true;
    }
    if ((first == last && !flag_moved_last) || (first != last)) {
        if (first <= m && s[0] != '1') {
            ans -= 1;
            m -= first;
        }
